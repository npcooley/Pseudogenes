...
AliRes <- SynRes <- vector(mode = "list",
                           length = length(FILES03))
...
DBPATH <- tempfile()
Seqs2DB(seqs = RefAssembly,
        dbFile = DBPATH,
        type = "XStringSet",
        verbose = TRUE,
        identifier = as.character(max(MAP1) + 1L))
...
# for (m1 in seq_along(FILES03)) {
  ...
  # load in stringset, if it's a single contig grab it's alignment
  if (file.exists(paste0("~/Data/20230509_SimulatedEcoliReassemblies",
                         "/",
                         JobMapB[MAP1[m1], 2L]))) {
    z1 <- readDNAStringSet(paste0("~/Data/20230509_SimulatedEcoliReassemblies",
                                  "/",
                                  JobMapB[MAP1[m1], 2L]))
    ...
    if (length(z1) == 1L) {
      Seqs2DB(seqs = z1,
              type = "XStringSet",
              dbFile = DBPATH,
              identifier = as.character(MAP1[m1]),
              verbose = FALSE)
      SynRes[[m1]] <- FindSynteny(dbFile = DBPATH,
                                  identifier = as.character(c(MAP1[m1], (max(MAP1) + 1L))),
                                  verbose = FALSE,
                                  allowOverlap = FALSE)
      AliRes[[m1]] <- DistanceMatrix(AlignSynteny(SynRes[[m1]],
                                                  verbose = FALSE,
                                                  dbFile = DBPATH)[[1]][[1]],
                                     verbose = FALSE,
