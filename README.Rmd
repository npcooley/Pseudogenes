...
```{r reviewer-request01, include = TRUE, echo = FALSE}
#| dev = c('png', 'pdf'), fig.width = 7, fig.height = 7, fig.align = "center",
#| fig.cap = "Assemblies generated from subsamples of available real reads on the SRA show increases in pseudogenization as subsamples become more sparse"

load("InputData/ReviewerRequest01Result_v01.RData",
     verbose = FALSE)

# simulated points to add to the plot...
sim_is_resp <- sim_fs_resp <- matrix(data = list(),
                                     nrow = 3,
                                     ncol = 3)
sim_plat_1 <- c("Illumina HiSeq 2500",
                "HiSeq X Ten",
                "Illumina MiSeq")
sim_plat_2 <- c("x",
                "MinION",
                "PacBio RS")
for (m1 in seq_len(nrow(is_vals05) / 2L)) {
  for (m2 in seq_len(ncol(is_vals05) / 3L)) {
    sim_is_resp[[m1, m2]] <- predictResponse(coef = is_vals05[m1 + 3L, c(m2, m2 + 3L, m2 + 6L)],
                                             COV = cov_set,
                                             QUAL = 0,
                                             TOT_GENE = tot_genes,
                                             TOT_NT = tot_nts)
    sim_fs_resp[[m1, m2]] <- predictResponse(coef = fs_vals05[m1 + 3L, c(m2, m2 + 3L, m2 + 6L)],
                                             COV = cov_set,
                                             QUAL = 0,
                                             TOT_GENE = tot_genes,
                                             TOT_NT = tot_nts)
...
s2 <- list(names(s1)[1],
           ...
           names(s1)[7],
           names(s1)[8],
           names(s1)[9:length(s1)])
labs01 <- paste0(c("E. coli (Simulated)",
                   "C. jejuni",
                   "L. monocytogenes",
                   "S. aureus",
                   "C. coli",
...
plot(x = 0,
     ...
     ylab = expression("|"*Delta*" pseudogenes| per Mbp at 50-fold coverage"),
     xaxs = "i",
     yaxs = "i",
     xaxt = "n",
     main = "Frameshifts")
...
for (m1 in seq_len(nrow(is_vals05) / 2L)) {
  for (m2 in seq_len(ncol(is_vals05) / 3L)) {
    ph_val <- sim_fs_resp[[m1, m2]][101]
    points(x = 1,
           y = ph_val,
           col = ColVec1[match(x = sim_plat_1[m1],
                               table = u_meta_1)],
           pch = match(x = sim_plat_2[m2],
                       table = u_meta_2))
    # print(ph_val)
  }
}

for (m1 in seq_len(length(labs01) - 1L)) {
  ...
  jitterpoints <- rnorm(n = length(w1),
                        mean = mx,
                        sd = 0.10)
  if (any(jitterpoints >= mx + .2)) {
    jitterpoints[jitterpoints >= mx + .2] <- mx
  }
  if (any(jitterpoints <= mx - .2)) {
    jitterpoints[jitterpoints <= mx - .2] <- mx
  }
...
for (m1 in seq_len(nrow(is_vals05) / 2L)) {
  for (m2 in seq_len(ncol(is_vals05) / 3L)) {
    ph_val <- convertCoverage(cov = cov_set[which.min(abs(sim_fs_resp[[m1, m2]] - 1))])
    points(x = 1,
           y = ph_val,
           col = ColVec1[match(x = sim_plat_1[m1],
                               table = u_meta_1)],
...
for (m1 in seq_len(length(labs01) - 1L)) {
  ...
  transf_y_vals[transf_y_vals > 400] <- 490
...
for (m1 in seq_len(nrow(is_vals05) / 2L)) {
  for (m2 in seq_len(ncol(is_vals05) / 3L)) {
    ph_val <- convertCoverage(cov = cov_set[which.min(abs(sim_is_resp[[m1, m2]] - 1))])
    points(x = 1,
           y = ph_val,
           col = ColVec1[match(x = sim_plat_1[m1],
                               table = u_meta_1)],
